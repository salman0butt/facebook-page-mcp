...
import type { PublishResult } from './facebook.js';
...
function publishPermissionHint(body: GraphErrorBody): string | undefined {
  ...
  if (body.error?.code === 200 && message.includes('publish_actions')) {
    return (
      'This MCP does not request publish_actions. Meta returned a legacy permission message for the Page write. ' +
      'Verify that the User token used to obtain this Page Access Token was granted pages_manage_posts and pages_read_engagement, then obtain a fresh Page Access Token for the Page from /me/accounts.'
    );
  }

  if (body.error?.code === 200) {
    return (
      'Facebook Page publishing requires Page publishing authorization such as pages_manage_posts and pages_read_engagement, plus content-creation access to the Page.'
    );
  }
...
async function assertConfiguredPageReadable(): Promise<void> {
  const url = graphUrl();
  ...
  if (!response.ok || body.error) {
    throw new Error(
      `Configured token cannot access Facebook Page ${config.pageId}: ${graphErrorMessage(response, body)}`
    );
  }

  if (!body.id || body.id !== config.pageId) {
    throw new Error(
      `Configured Facebook Page check returned unexpected id ${body.id ?? 'missing'}; expected ${config.pageId}.`
    );
  }
}
...
export async function publishTextPost(message: string): Promise<PublishResult> {
  ...
  await assertConfiguredPageReadable();
...
export async function publishPhotoFromUrl(
  ...
  const response = await graphFetch(endpoint, {
    method: 'POST',
    headers: graphHeaders('application/x-www-form-urlencoded'),
    body: new URLSearchParams({ url: imageUrl, caption, published: 'true' })
  });
  const result = await parsePublishResponse(response);
